<template>
  <div class="bg-[#e6e6e6] p-5 rounded-[20px] max-w-[420px] mx-auto mb-6 text-center">
    <h3 class="mb-3 text-[17px] font-bold">Mark your attendance for today</h3>
    ...
    <span
      :class="[
        ...
        status === 'checked' ? 'bg-[#2d8a4b] text-white' : status === 'checked-out' ? 'bg-[#b5b5b5] text-white' : 'bg-[#b5b5b5] text-white',
      ]"
    >
      {{ statusText }}
    </span>

    <p v-if="checkinTime" class="mt-2.5 font-medium">Checked in at: {{ checkinTime }}</p>

    <p v-if="elapsedTime" class="mt-1.5 text-sm font-semibold text-[#444]">
      {{ elapsedTime }}
    </p>
