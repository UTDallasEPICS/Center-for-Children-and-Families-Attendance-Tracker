export default defineEventHandler(async (event) => {
    ...
    const fields = ["record_id", "id", "chname_reg", "chlname_reg", "chdob_reg", "mphone_reg", "memail_reg", "childatt_checkin"]
    ...
    const participants = (redcapData as any[]).map((row) => {
        ...
        const status = row.childatt_checkin === "1" ? "checked-in" : row.childatt_checkin === "0" ? "pending" : "unknown"
